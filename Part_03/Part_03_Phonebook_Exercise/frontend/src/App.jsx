...
const App = () => {
  ...
  const updatePhoneBook = (event) => {
    ...
    if (data !== false) {
      ...
      const decision = confirm(
        `${newName} is alrerady added to the phonebook, replace the old number with new one?`,
      );
  ...
  const updateNumber = (existingEntry) => {
    ...
    phoneBookServices.update(existingEntry.id, updatedEntry).then((res) => {
  ...
  const destroyPhoneBookEntry = (id) => {
    phoneBookServices.destroyPhoneEntry(id).then((res) => {
      ...
      <ul>
        ...
        {persons.map((person) => (
          <Names
            ...
            shouldDelete={() => destroyPhoneBookEntry(person.id)}
          />
        ))}
      </ul>
    </div>
